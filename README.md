...
Telegram Mini App по мотивам **Steal a Brainrot** (Roblox): кейсы с брейнротами,
внутренняя валюта, бонусы, промокоды, реферальная программа, пополнение, вывод
средств и админ-панель.

Контент повторяет устройство игры: восемь игровых тиров редкости
(Common → Rare → Epic → Legendary → Mythic → Brainrot God → Secret → OG) и кейсы,
собранные вокруг её механик — конвейер, кражи с чужих баз и мутации
(Gold ×1.25, Diamond ×1.5, Bloodrot ×2, Candy ×4, Lava ×6, Rainbow ×10, Crystal ×13).
...
16. [Экономика и вероятности](#экономика-и-вероятности)
17. [Платёжные провайдеры](#платёжные-провайдеры)
...
| 🎁 Кейсы | 14 кейсов по тирам и мутациям Steal a Brainrot, анимация открытия |
...
npm run db:seed          # 14 кейсов, 110 наград, 4 промокода, админ
```
...
После seed доступны промокоды `WELCOME`, `SAHUR`, `RAINBOW`, `OGDROP`.
...
```bash
npm test              # весь набор: 75 тестов
npm run test:shared   # юнит-тесты уровней и форматирования
npm run test:server   # юниты + интеграционные тесты API
```

Интеграционные тесты поднимают приложение через `fastify.inject()` и работают с
отдельной базой `nexus_test` — рабочие данные они не трогают (перед очисткой
проверяется, что в имени базы есть `test`). Миграции применяются автоматически
перед прогоном, адрес переопределяется переменной `TEST_DATABASE_URL`.

Что покрыто:

| Группа | Проверяется |
| --- | --- |
| Уровни и формат | пороги XP, прогресс-бар, форматирование валюты, маскирование реквизитов |
| Weighted random | сходимость распределения к заданным весам, игнор нулевых весов, ненулевые коллизии кодов |
| Telegram initData | корректная подпись, подделка хеша, подмена `user` при валидной подписи, просроченный `auth_date` |
| Открытие кейса | списание и начисление одной операцией, лента анимации, нехватка средств, выключенный кейс |
| **Гонки** | 8 параллельных открытий при балансе на одно → ровно одно успешное, баланс не уходит в минус |
| Бесплатный кейс | кулдаун 24 часа, отсутствие двойного открытия при параллельных запросах, доступность после истечения |
| Бонусы | ежедневный выдаётся раз в сутки, параллельные запросы не удваивают, незавершённая серия отклоняется |
| Промокоды | повторная активация, общий лимит, выключенный и несуществующий код |
| Рефералы | бонус обеим сторонам, самоприглашение, невозможность сменить реферера |
| Пополнение | зачисление только после webhook, идемпотентность, параллельные webhook, неверная подпись, бонус процента дня |
| Вывод | резерв средств, маскирование реквизитов, возврат при отказе, повторная обработка заявки |
| Права | админ-API закрыт для игроков, отзыв прав действует немедленно, бан блокирует игру, аудит изменений баланса |

CI (`.github/workflows/ci.yml`) на каждый push поднимает PostgreSQL и прогоняет
проверку типов, тесты, миграции, seed и production-сборку.
...
Тиры редкости повторяют игровые:

| Тир | Подпись в интерфейсе | Цвет | Примеры |
| --- | --- | --- | --- |
| Common | Обычный | серый | Noobini Pizzanini, Tim Cheese, Pipi Corni |
| Rare | Редкий | голубой | Trippi Troppi, Boneca Ambalabu, Brr Brr Patapim |
| Epic | Эпический | фиолетовый | Cappuccino Assassino, Bombardiro Crocodilo, Lirilì Larilà |
| Legendary | Легендарный | золотой | Ballerina Cappuccina, Chimpanzini Bananini, Chef Crabracadabra |
| Mythic | Мифический | розовый | Tralalero Tralala, Orcalero Orcala, Matteo |
| Brainrot God | Божество | изумрудный | Tung Tung Tung Sahur, Cocofanto Elefanto, Odin Din Din Dun |
| Secret | Секретный | красный | La Vacca Saturno Saturnita, Graipuss Medussi, Garama and Madundung |
| OG | OG | радужный градиент | Skibidi Toilet, Meowl, Strawberry Elephant, Spyder Elephant |

Все платные кейсы из seed настроены на **RTP ≈ 90%** (возврат игроку), то есть
маржа платформы составляет около 10% оборота:

| Кейс | Категория | Цена | Наград | Джекпот | RTP |
| --- | --- | --- | --- | --- | --- |
| 📦 КОНВЕЙЕР | бесплатный | бесплатно | 6 | 500 B | — |
| 🏠 ПЕРВАЯ БАЗА | обычный | 25 B | 8 | 950 B | 88% |
| 🌙 НОЧНАЯ КРАЖА | популярный | 120 B | 8 | 4 550 B | 90% |
| 🪙 ЗОЛОТАЯ ЖИЛА | популярный | 300 B | 8 | 11 500 B | 90% |
| 🍝 ИТАЛЬЯНСКИЙ КВАРТАЛ | тематический | 450 B | 8 | 17 000 B | 90% |
| 💎 АЛМАЗНАЯ ГРАНЬ | популярный | 700 B | 8 | 26 500 B | 90% |
| 🥁 НОЧЬ САХУРА | тематический | 900 B | 8 | 34 000 B | 90% |
| 🌊 ГЛУБИНА | тематический | 1 400 B | 8 | 53 000 B | 90% |
| 🌈 РАДУЖНЫЙ СБОЙ | популярный | 1 800 B | 8 | 68 500 B | 90% |
| 👑 БОЖЕСТВО РОТА | дорогой | 3 500 B | 8 | 135 000 B | 90% |
| ⚡ АДМИН-ИВЕНТ | тематический | 5 000 B | 8 | 190 000 B | 90% |
| 🗝 СЕКРЕТНЫЙ АРХИВ | дорогой | 9 000 B | 8 | 340 000 B | 90% |
| 🔮 КРИСТАЛЛ 13X | дорогой | 20 000 B | 8 | 760 000 B | 90% |
| 🏛 OG ХРАНИЛИЩЕ | дорогой | 50 000 B | 8 | 1 900 000 B | 90% |

Награда каждого кейса — конкретный брейнрот со своим тиром: от `Noobini Pizzanini`
и `Tim Cheese` в стартовых до `La Grande Combinasion`, `Garama and Madundung` и
OG-тира (`Skibidi Toilet`, `Meowl`, `Strawberry Elephant`, `Spyder Elephant`) в топовых.
Мутационные кейсы выдают те же персонажи с префиксом мутации.

Лестница наград во всех платных кейсах одинаковая по форме: младшая награда ≈ 0.15
от цены и выпадает примерно в 41% случаев, джекпот — 38× цены с шансом ~0.16%.
Изменить любую вероятность можно из админ-панели — она сразу показывает пересчитанный RTP.
